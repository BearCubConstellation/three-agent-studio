...
- 受控、可校验的场景操作协议：几何体、材质、灯光、相机、删除、撤销、重做
- 场景树、视口点选、对象属性面板与 TransformControls 拖拽变换
...
- **导出 glTF / GLB**：只会导出可编辑的场景对象，不包含编辑器网格和变换控件等辅助对象。
- **视口变换**：选中对象后可在顶部切换移动、旋转、缩放模式；一次拖拽会记为一条可撤销事务。
...
| `scene_get_tree` | 获取场景树、对象 UUID、层级和选中状态 |
| `scene_get_selected` | 获取当前选中对象的变换和材质属性 |
| `scene_select` | 按 UUID 在浏览器编辑器中选中对象 |
| `scene_apply_ops` | 批量执行声明式场景操作 |
| `scene_validate` | 检查灯光、网格、复杂度、材质 |
| `scene_undo` | 撤销最近一笔场景事务 |
| `scene_redo` | 重做最近一笔被撤销的场景事务 |
